function wpli_matrix = wpli_sin(hilbert_matrix, vec_voxels)
...
for i = vec_voxels
   for k = vec_voxels
       if k == i
           ...
           num = abs(mean(sin(angle(hilbert_matrix(:,i))- angle(hilbert_matrix(:,k)))));
           wpli_value = num / mean(abs(sin(angle(hilbert_matrix(:,i))-angle(hilbert_matrix(:,k)))));
