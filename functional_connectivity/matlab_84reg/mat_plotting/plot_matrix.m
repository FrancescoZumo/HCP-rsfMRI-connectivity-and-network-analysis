function plot_matrix(matrix, figure_name, color_range, voxels, vet_dist, vet_label, vet_color, vet_cum)
...
% vedere nel plot.
vet_dist = (1:length(vet_dist));
vet_label = zeros(1,length(vet_label));
for m = 1:length(vet_label)
   if m == 1
       vet_label(m) = 0;
   elseif m == length(vet_label)
       vet_label(m) = 1;
   else
       vet_label(m) = NaN;
   end 
...
hold on;
for i = 1:voxels+1
   plot([.5,voxels+.5],[i-.5,i-.5],'k-');
   plot([i-.5,i-.5],[.5,voxels+.5],'k-');
end
...
c1 = colorbar(ax1, 'eastoutside', 'Ticks', color_range, 'TickLabels', color_range, 'FontSize', 12);
caxis(ax1,color_range);
c2 = colorbar(ax2, 'westoutside', 'Direction', 'reverse', 'XTick', [], 'XTickLabel', []); 
caxis(ax2, [1 voxels+1]);
c3 = colorbar(ax3, 'southoutside', 'XTick', [], 'XTickLabel', []); 
