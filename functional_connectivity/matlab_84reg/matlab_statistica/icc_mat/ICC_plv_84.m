function [ICC_matrix] = ICC_plv_84(voxels)
...
for i = 1:voxels
    for k = 1:i
        for m = 1:n_soggetti
            ...
            std_inter_array(1,m) = (plv_matrix_s1200_norm(i,k) + plv_matrix_sretest_norm(i,k))/2;
            % Faccio std tra medesima connettività regione del medesimo soggetto nelle
            % due sessioni
            std_intra_array(1,m) = (plv_matrix_s1200_norm(i,k) - plv_matrix_sretest_norm(i,k))^2;
        end
