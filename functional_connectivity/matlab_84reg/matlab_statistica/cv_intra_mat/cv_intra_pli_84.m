function [CV_intra_matrix] = cv_intra_pli_84(voxels)
    ...
    for k = 1:i
        for m = 1:n_soggetti
           % Carico soggetto s1200
            ...
            media_CVintra_array(1,m) = mean([pli_matrix_s1200_norm(i,k), pli_matrix_sretest_norm(i,k)]);
            % Calcolo per varianza inter-soggetto (sigma_ws)
            sigws_array(1,m) = (pli_matrix_s1200_norm(i,k) - pli_matrix_sretest_norm(i,k))^2;
        end
