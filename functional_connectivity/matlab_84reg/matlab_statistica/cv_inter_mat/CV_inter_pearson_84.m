function [CV_inter_1200_matrix, CV_inter_retest_matrix, CV_inter_matrix] = CV_inter_pearson_84(voxels)
    ...
    for k = 1:i
        for m = 1:n_soggetti
           % Carico soggetto s1200
            ...
            media_CVinter_array(1,m) = corr_pearson_s1200_valzero_norm(i,k);
            % Calcolo per varianza inter-soggetto (sigma_ws)
            sig_array(1,m) = corr_pearson_s1200_valzero_norm(i,k);
        end
...
for i = 1:voxels
    for k = 1:i
        for m = 1:n_soggetti
            ...
            % Vettore che serve per l'overall mean
            media_CVinter_array(1,m) = corr_pearson_sretest_valzero_norm(i,k);
            % Calcolo per varianza inter-soggetto (sigma_ws)
            sig_array(1,m) = corr_pearson_sretest_valzero_norm(i,k);
        end
