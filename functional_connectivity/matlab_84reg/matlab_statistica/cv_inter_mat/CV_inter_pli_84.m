function [CV_inter_1200_matrix, CV_inter_retest_matrix, CV_inter_matrix] = CV_inter_pli_84(voxels)
...
for i = 1:voxels
    for k = 1:i
        for m = 1:n_soggetti
            ...
            media_CVinter_array(1,m) = pli_matrix_s1200_norm(i,k);
            % Calcolo per varianza inter-soggetto (sigma_ws)
            sig_array(1,m) = pli_matrix_s1200_norm(i,k);
        end
...
for i = 1:voxels
    for k = 1:i
        for m = 1:n_soggetti
            ...
            load(percorso); 
            % Vettore che serve per l'overall mean
            media_CVinter_array(1,m) = pli_matrix_sretest_norm(i,k);
            % Calcolo per varianza inter-soggetto (sigma_ws)
            sig_array(1,m) = pli_matrix_sretest_norm(i,k);
        end
