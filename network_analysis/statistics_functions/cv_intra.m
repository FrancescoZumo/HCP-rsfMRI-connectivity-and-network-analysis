function [CV_intra_matrix] = cv_intra_wpli_84(voxels)
...
for i = 1:voxels
    for k = 1:i
        for m = 1:n_soggetti
            ...
            media_CVintra_array(1,m) = mean([wpli_matrix_s1200_norm(i,k), wpli_matrix_sretest_norm(i,k)]);
            % Calcolo per varianza inter-soggetto (sigma_ws)
            sigws_array(1,m) = (wpli_matrix_s1200_norm(i,k) - wpli_matrix_sretest_norm(i,k))^2;
        end
